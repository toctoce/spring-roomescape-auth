...
class ReservationControllerTest {
    ...
    @Test
    void 회원_id로_예약_목록을_조회한다() {
        Map<String, Object> params1 = new HashMap<>();
        params1.put("memberId", 2L);
    ...
    @Test
    void 회원_id_없이_예약_목록을_조회하면_400을_응답한다() {
        RestAssured.given().log().all()
                .when().get("/reservations")
                .then().log().all()
                .statusCode(400)
                .body("message", is("memberId: 입력값이 필요합니다."));
    }
    ...
    @Test
    void 회원_id가_다르면_예약_삭제_시_403을_응답한다() {
        Map<String, Object> params = new HashMap<>();
        params.put("memberId", 2L);
        params.put("date", "2099-05-03");
        params.put("timeId", 1L);
    ...
    @Test
    void 예약_삭제_시_회원_id를_입력하지_않으면_400을_응답한다() {
        RestAssured.given().log().all()
    ...
    @Test
    void 회원_id가_다르면_예약_수정_시_403을_응답한다() {
        Map<String, Object> params = new HashMap<>();
        params.put("memberId", 2L);
        params.put("date", "2099-05-03");
        params.put("timeId", 1L);
    ...
    @ParameterizedTest(name = "{0}은 예약자 회원 id 형식이 아니다")
    @ValueSource(strings = {"", "abc"})
    void 예약을_추가할_때_회원_id_형식이_올바르지_않으면_400을_응답한다(String memberId) {
        Map<String, Object> params = new HashMap<>();
