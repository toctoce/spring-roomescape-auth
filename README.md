...
- [x] 인증 실패와 인가 실패를 구분한다.
  - [x] 비로그인 요청은 `401 Unauthorized`로 처리한다.
  - [x] 매니저가 아닌 사용자나 다른 매장 예약 접근은 `403 Forbidden`으로 처리한다.
  - [x] 인가 판단 로직은 컨트롤러에 흩어지지 않도록 서비스 계층에 둔다.

- [x] 인가 테스트를 작성한다.
  - [x] 비로그인 사용자의 관리자 예약 API 접근 실패를 검증한다.
  - [x] 매니저가 아닌 사용자의 관리자 예약 API 접근 실패를 검증한다.
  - [x] 매니저가 자기 매장 예약을 조회, 변경, 삭제할 수 있는지 검증한다.
  - [x] 매니저가 다른 매장 예약을 조회, 변경, 삭제할 수 없는지 검증한다.
